...
with nav1:
    ...
    st.page_link("pages/page1_shipper_tracking.py", label="열기 →", icon="📦")
with nav2:
    st.markdown("#### 🚚 트럭기사용 앱")
    st.caption("배차 안내, 공차 방지 복귀화물 매칭")
    st.page_link("pages/page2_driver_app.py", label="열기 →", icon="🚚")
with nav3:
    st.markdown("#### 🛰️ 관제센터")
    st.caption("전체 예약 현황, 탄소절감·리드타임 KPI")
    st.page_link("pages/page3_control_tower.py", label="열기 →", icon="🛰️")
with nav4:
    st.markdown("#### 🚃 화차 배치 추천")
    st.caption("서모게이트 ML 모델로 화물-화차 매칭")
    st.page_link("pages/page4_car_assignment.py", label="열기 →", icon="🚃")
