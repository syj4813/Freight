...
app.py에서 "철도 통합운송"으로 예약 확정한 화물만 여기 표시된다.
단계(현재단계)는 random이 아니라 shared_store.current_stage_idx()가
예약시각~도착예정시각 경과 비율로 결정론적으로 계산한다.
"""
...
if not shipments:
    st.info(
        "아직 예약된 화물이 없습니다. 메인 페이지(app.py)에서 견적 비교 후 "
        "'철도 통합운송'을 예약 확정하면 여기에 표시됩니다."
    )
    st.page_link("pages/page0_home.py", label="메인으로 이동 →", icon="🏠")
    st.stop()
...
if eta:
    remaining = int((eta - datetime.now()).total_seconds() // 60)
...
with d2:
    ...
    fare = record.get("요금원")
    st.write(f"**요금**: {fare:,}원" if isinstance(fare, (int, float)) else f"**요금**: {fare}")
...
st.caption(
    "※ 진행 단계는 실제 GPS/RFID 트래킹이 아니라, 예약시각과 도착예정시각 사이의 "
    "경과 비율로 추정한 시뮬레이션입니다."
)
