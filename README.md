...
화주의 견적 비교(트럭 vs 철도 통합운송, 500kg 이상)부터, 철도 통합운송 예약 확정 이후의
...
- `app.py` — 진입점(라우터). `st.navigation()`으로 아래 화면들을 연결 (2026-08-08부터: 예전엔 app.py 자체가 화주용 화면이었으나, 한글 파일명 문제로 파일 구조를 정리하며 분리)
- `pages/page0_home.py` — 화주용 견적 비교(트럭 vs 철도 통합운송, 500kg 이상) + 철도 통합운송 예약 확정 (구 app.py 내용)
- `pages/page1_shipper_tracking.py` — 예약 확정된 화물의 door-to-door 진행 상황 추적
- `pages/page2_driver_app.py` — 배차 안내 + 복귀 화물(공차 방지) 매칭
- `pages/page3_control_tower.py` — 전체 예약 현황, 탄소절감·리드타임 등 실측 KPI
- `pages/page4_car_assignment.py` — 서모게이트 ML 모델(LightGBM) 기반 화물-화차 매칭 추천
...
- 2026-08-08부터 퀵서비스·KTX특송 취급 중단 (500kg 미만 소형화물 전용이라 서비스 방향과 안 맞아 제거) — `ktx_tucking.py` 파일도 함께 삭제, `road_cost.py`의 `estimate_quick_fare`는 더 이상 호출되지 않지만 다른 소형화물 서비스에서 재사용할 수 있어 남겨둠
- 트럭 단독 예약 건은 화물역 환적 구간이 없어 실시간 추적·트럭기사 연계 대상에서 제외됨 (철도 통합운송 건만 후단 화면과 연동)
